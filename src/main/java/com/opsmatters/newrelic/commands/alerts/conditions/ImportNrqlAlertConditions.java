...
/**
 * Implements the New Relic command line option to import a set of NRQL alert conditions.
 * 
 * @author Gerald Curley (opsmatters)
 */
public class ImportNrqlAlertConditions extends BaseCommand
{
    private static final Logger logger = Logger.getLogger(ImportNrqlAlertConditions.class.getName());
    private static final String NAME = "import_nrql_alert_conditions";
     ...
     * Default constructor.
     */
    public ImportNrqlAlertConditions()
    {
        options();
    }
    ...
    /**
     * Import the NRQL alert conditions.
     */
    protected void execute()
    {
        ...
        try
        {
            ...
            config.setNrqlAlertConditions(manager.readNrqlAlertConditions(policies, filename, worksheet, 
                new FileInputStream(filename)));
        }
        catch(IOException e)
        {
            logger.severe("Unable to read NRQL alert condition file: "+e.getClass().getName()+": "+e.getMessage());
        }

        // Delete the existing alert conditions
        if(delete)
            manager.deleteNrqlAlertConditions(config.getNrqlAlertConditions());

        // Create the new alert conditions
        manager.createNrqlAlertConditions(config.getNrqlAlertConditions());
    }
}