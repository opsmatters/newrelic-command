...
/**
 * Implements the New Relic command line option to import a set of infrastructure host alert conditions.
 * 
 * @author Gerald Curley (opsmatters)
 */
public class ImportInfraHostNotReportingAlertConditions extends BaseCommand
{
    private static final Logger logger = Logger.getLogger(ImportInfraHostNotReportingAlertConditions.class.getName());
    private static final String NAME = "import_infra_host_conditions";
     ...
     * Default constructor.
     */
    public ImportInfraHostNotReportingAlertConditions()
    {
        options();
    }
    ...
    /**
     * Import the infrastructure host alert conditions.
     */
    protected void execute()
    {
        ...
        try
        {
            ...
            config.setInfraAlertConditions(manager.readInfraHostNotReportingAlertConditions(policies, filename, worksheet, 
                new FileInputStream(filename)));
        }
        catch(IOException e)
        {
            logger.severe("Unable to read infra host alert condition file: "+e.getClass().getName()+": "+e.getMessage());
        }

        // Delete the existing alert conditions
        if(delete)
            manager.deleteInfraAlertConditions(config.getInfraHostNotReportingAlertConditions());

        // Create the new alert conditions
        manager.createInfraAlertConditions(config.getInfraHostNotReportingAlertConditions());
    }
}