...
/**
 * Implements the New Relic command line option to import a set of external service alert conditions.
 * 
 * @author Gerald Curley (opsmatters)
 */
public class ImportExternalServiceAlertConditions extends BaseCommand
{
    private static final Logger logger = Logger.getLogger(ImportExternalServiceAlertConditions.class.getName());
    private static final String NAME = "import_external_service_alert_conditions";
     ...
     * Default constructor.
     */
    public ImportExternalServiceAlertConditions()
    {
        options();
    }
    ...
    /**
     * Import the external service alert conditions.
     */
    protected void execute()
    {
        ...
        try
        {
            ...
            config.setExternalServiceAlertConditions(manager.readExternalServiceAlertConditions(policies, entities, filename, worksheet, 
                new FileInputStream(filename)));
        }
        catch(IOException e)
        {
            logger.severe("Unable to read external service alert condition file: "+e.getClass().getName()+": "+e.getMessage());
        }

        // Delete the existing alert conditions
        if(delete)
            manager.deleteExternalServiceAlertConditions(config.getExternalServiceAlertConditions());

        // Create the new alert conditions
        manager.createExternalServiceAlertConditions(config.getExternalServiceAlertConditions());
    }
}