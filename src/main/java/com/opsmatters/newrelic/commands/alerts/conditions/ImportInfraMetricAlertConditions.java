...
/**
 * Implements the New Relic command line option to import a set of infrastructure metric alert conditions.
 * 
 * @author Gerald Curley (opsmatters)
 */
public class ImportInfraMetricAlertConditions extends BaseCommand
{
    private static final Logger logger = Logger.getLogger(ImportInfraMetricAlertConditions.class.getName());
    private static final String NAME = "import_infra_metric_conditions";
     ...
     * Default constructor.
     */
    public ImportInfraMetricAlertConditions()
    {
        options();
    }
    ...
    /**
     * Import the infrastructure metric alert conditions.
     */
    protected void execute()
    {
        ...
        try
        {
            ...
            config.setInfraAlertConditions(manager.readInfraMetricAlertConditions(policies, filename, worksheet, 
                new FileInputStream(filename)));
        }
        catch(IOException e)
        {
            logger.severe("Unable to read infra metric alert condition file: "+e.getClass().getName()+": "+e.getMessage());
        }

        // Delete the existing alert conditions
        if(delete)
            manager.deleteInfraAlertConditions(config.getInfraMetricAlertConditions());

        // Create the new alert conditions
        manager.createInfraAlertConditions(config.getInfraMetricAlertConditions());
    }
}